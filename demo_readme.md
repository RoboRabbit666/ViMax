...
本文档通过一次真实的 demo 运行，逐阶段讲解 ViMax `idea2video` 流水线的工作方式。目标是展示：一个纯文字提示如何经过一系列结构化的自动化步骤，最终生成一段完整的视频。

演示源文件位于 [`demo/ViMax_output/`](demo/ViMax_output/)。

---

## 输入

整个流程的起点是一段自然语言描述，包含故事类型、基调、角色和戏剧意图。无需提供任何图像、脚本或手工分镜。

**本次 Demo 的输入（概要）：**

> *暗黑奇幻赛博朋克短片。面向成年观众。单场景。在废墟城市 Neo-Veridia，独行战士 Kael 与一个无名神秘人物"先知（The Oracle）"展开最后的对决。*
...
流水线将输入提示展开为一篇完整叙事，建立世界观、角色设定和逐镜头动作描述，作为所有下游阶段的创作基础。

**Demo 节选：**

> *在 Neo-Veridia 的残骸之中，笼罩在腐蚀性烟雾与垂死的霓虹灯下，战士 Kael 与那个只被称为"先知"的神秘人物展开最后的对决。Kael 身负绑缚在剑上的古老发光技术，寻求一个答案——或许只是为了给她毫无结果的朝圣之旅画上一个明确的句号。*

本次 Demo 共定义了三个镜头：
- **镜头 1** — 超大远景，Kael 进入废弃街道
- **镜头 2** — 中近景，她拔出 Luminar 光刃，与先知对峙
...
叙事被重新结构化为机器可读的剧本格式：场景标题、动作描述、角色台词和音效说明。视觉描述与音频线索在此阶段分离，以便后续各自独立处理。
...
每个场景被分解为若干镜头，每个镜头包含明确的 `visual_desc`（视觉描述）和 `audio_desc`（音频描述）字段，以及该镜头中出现的角色索引（`vis_char_idxs`）和是否为场景末镜（`is_last`）。
...
- **镜头 3：** 镜头快速推进至冲击点。刺眼白光吞噬画面，声音骤然切断。
...
所有场景视频拼接成最终输出。本次 Demo 只有一个场景，因此 `final_video.mp4` 与 `scene_0/final_video.mp4` 等价。
